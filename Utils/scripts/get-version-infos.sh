...
find /opt/centralist/ -name "package.json" 2>/dev/null | xargs grep "\"~" | sed "s/\"//g" | sed "s/,//g" | sed "s/:/;/g" | awk '{print "NPM;"$1""$2""$3""$4}'   | grep -v 'node_modules\|bower_components' >> "$CONF_FILE"

echo "Extraction des dépendances dans les fichiers bower.json du projet"
find /opt/centralist/ -name "bower.json"   2>/dev/null | xargs grep "\"~" | sed "s/\"//g" | sed "s/,//g" | sed "s/:/;/g" | awk '{print "BOWER;"$1""$2""$3""$4}' | grep -v 'node_modules\|bower_components' >> "$CONF_FILE"
